...
class DeviceManager:
    ...
    SELECTOR_OPTIONS = {
        "selector_security_lock": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Locked|Unlocked",
        },
        "selector_open_closed": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Closed|Open",
        },
        "selector_on_off": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Off|On",
        },
        "selector_vehicle_state": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Parked|Moving|Other",
        },
        "selector_security": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Secure|Attention",
        },
        "selector_climate": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Off|Heating|Cooling|Ventilation|Climate",
        },
        "selector_charging": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Not Charging|Charging|Complete|Scheduled|Error",
        },
        "selector_connected": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Disconnected|Connected",
        },
        "selector_charge_mode": {
            "SelectorStyle": "0",
            "LevelOffHidden": "false",
            "LevelNames": "Unknown|Immediate|Scheduled|Manual|Other",
        },
    }
    ...
    def _create(self, unit, name, type_name, mode):
        """Ensure one device exists with the desired native Domoticz type.

        Existing devices are migrated in-place.

        This deliberately avoids the old Delete() -> Create() migration
        sequence. Deleting first could remove the existing device and its
        history if Domoticz crashed before the replacement was created.

        In-place Update() preserves the existing device, IDX and history.
        """
        ...
        if device is not None:
            if self._needs_migration(device, desired):
                if not self._migrate_in_place(
                    ...
                    # Migration failed. Keep the existing device intact.
                    return device
            else:
        ...
        # Device genuinely does not exist: create it normally.
        try:
            kwargs = {
                "Name": name,
                "Unit": unit,
    ...
    def _synchronize_options(self, unit, device, desired):
        """Keep Domoticz options synchronized without changing the value."""
        ...
        try:
            ...
            self._log_error(
                "Could not configure unit {}: {}".format(
                    unit,
                    exc,
                )
            )
    ...
    def _migrate_in_place(self, unit, name, type_name, device, desired):
        """Change Type/SubType/SwitchType without deleting the device.

        Existing device values, IDX and history are retained.

        The legacy Domoticz Python API exposes existing device properties as:

            Type
            SubType
            SwitchType

        Device.Create()/Update() use the corresponding keyword names:
        ...
        try:
            self.logger.Log(
                "Migrating unit {} ({}) to native {} in-place".format(
                    unit,
                    name,
                    type_name,
                )
            ...
            kwargs = {
                ...
                "SwitchType": desired["Switchtype"],
            }

            if desired.get("Options"):
                kwargs["Options"] = desired["Options"]

            # Keep the current value while changing the device definition.
            #
            # SuppressTriggers avoids treating the type migration as a
            # telemetry update.
            kwargs["SuppressTriggers"] = True
            ...
            self._log_error(
                "Could not migrate unit {} ({}) in-place: {}".format(
                    unit,
                    name,
                    exc,
                )
    ...
    def _desired_definition(self, mode):
        ...
        if mode == "custom_c":
            return {
                "Type": self.CUSTOM_C_TYPE,
                "Subtype": self.CUSTOM_C_SUBTYPE,
                "Switchtype": self.CUSTOM_C_SWITCHTYPE,
                "Options": self.CUSTOM_C_OPTIONS,
            }
        ...
        if mode == "custom_seconds":
            return {
                ...
                "Subtype": self.CUSTOM_KM_SUBTYPE,
                "Switchtype": self.CUSTOM_KM_SWITCHTYPE,
                "Options": self.CUSTOM_SECONDS_OPTIONS,
            }
        ...
        if mode == "custom_days":
            return {
                ...
                "Subtype": self.CUSTOM_KM_SUBTYPE,
                "Switchtype": self.CUSTOM_KM_SWITCHTYPE,
                "Options": self.CUSTOM_DAYS_OPTIONS,
            }
        ...
        if mode == "custom_requests":
            return {
                ...
                "Subtype": self.CUSTOM_KM_SUBTYPE,
                "Switchtype": self.CUSTOM_KM_SWITCHTYPE,
                "Options": self.CUSTOM_REQUESTS_OPTIONS,
            }
    ...
    def _needs_migration(device, desired):
        """Return True when the existing native device definition differs.

        Domoticz exposes the legacy Python Device attributes as:

            Type
            SubType
            SwitchType

        The desired definitions use:

            Type
            Subtype
            Switchtype

        The previous implementation incorrectly queried ``device.Subtype``.
        The correct legacy API property is ``device.SubType``.

        That typo caused every selector/custom device to appear different
        after every restart, resulting in repeated Delete()/Create()
        operations.
        """
        checks = (
            ("Type", "Type"),
        ...
        for device_attr, desired_key in checks:
            if desired_key in desired:
                if getattr(device, device_attr, None) != desired[desired_key]:
                    return True

        # Text devices have no native Type/SubType/SwitchType migration
        # requirement here.
        return False

    def ensure_devices(self):
        """Ensure all defined devices exist and have the correct type."""
    ...
    def _update_selector(self, key, value):
        ...
        level = self.SELECTOR_LEVELS.get(
            text,
            self.SELECTOR_LEVELS.get(
                text.upper(),
                0,
            ),
        )
    ...
    def _selector_mode(key):
        return {
            "doors_locked": "selector_security_lock",
            "doors": "selector_open_closed",
            "windows": "selector_open_closed",
            "lights": "selector_on_off",
            "trunk": "selector_open_closed",
            "bonnet": "selector_open_closed",
            "sunroof": "selector_open_closed",
            "vehicle_state": "selector_vehicle_state",
            "air_conditioning": "selector_on_off",
            "auxiliary_heating": "selector_on_off",
            "active_ventilation": "selector_on_off",
            "vehicle_security": "selector_security",
            "climate_state": "selector_climate",
            "charging_state": "selector_charging",
            "charging_connected": "selector_connected",
            "charge_mode": "selector_charge_mode",
        }[key]
    ...
    def restore_selector(self, unit, state):
        reverse = {
            UNITS["doors_locked"]: (
                "doors_locked",
                "LOCKED"
                if state.doors_locked is True
                else (
                    "UNLOCKED"
                    if state.doors_locked is False
                    else "UNKNOWN"
                ),
            ),
            UNITS["doors"]: (
                "doors",
                self._normalized(state.doors),
            ),
            UNITS["windows"]: (
                "windows",
                self._normalized(state.windows),
            ),
            UNITS["lights"]: (
                "lights",
                self._normalized(state.lights),
            ),
            UNITS["trunk"]: (
                "trunk",
                self._normalized(state.trunk),
            ),
            UNITS["bonnet"]: (
                "bonnet",
                self._normalized(state.bonnet),
            ),
            UNITS["sunroof"]: (
                "sunroof",
                self._normalized(state.sunroof),
            ),
            UNITS["vehicle_state"]: (
                "vehicle_state",
                self._smart_vehicle_state(state),
            ),
            UNITS["air_conditioning"]: (
                "air_conditioning",
                self._normalized(state.air_conditioning),
            ),
            UNITS["auxiliary_heating"]: (
                "auxiliary_heating",
                self._normalized(state.auxiliary_heating),
            ),
            UNITS["active_ventilation"]: (
                "active_ventilation",
                self._normalized(state.active_ventilation),
            ),
            UNITS["vehicle_security"]: (
                "vehicle_security",
                self._smart_security(state),
            ),
            UNITS["climate_state"]: (
                "climate_state",
                self._smart_climate(state),
            ),
            UNITS["charging_state"]: (
                "charging_state",
                self._normalized(state.charging_state),
            ),
            UNITS["charging_connected"]: (
                "charging_connected",
                "CONNECTED"
                if state.charging_connected
                else (
                    "DISCONNECTED"
                    if state.charging_connected is False
                    else "UNKNOWN"
                ),
            ),
            UNITS["charge_mode"]: (
                "charge_mode",
                self._normalized(state.charge_mode),
            ),
        }
    ...
    def _update_percentage(self, key, value):
        ...
        self._update(
            key,
            int(round(number)),
            "{:.1f}".format(number).rstrip("0").rstrip("."),
        )
    ...
    def _smart_climate(cls, state):
        heating = cls._normalized(
            state.auxiliary_heating
        )
        ventilation = cls._normalized(
            state.active_ventilation
        )
        ac = cls._normalized(
    ...
    def _smart_vehicle_state(cls, state):
        parking = cls._normalized(
            state.parking_state
        )

        vehicle = cls._normalized(
            state.vehicle_state
        )
    ...
    def _capabilities(state):
        ...
        if (
            ...
            or state.charging_state not in (
                "UNKNOWN",
                "",
            )
        ):
        ...
        if state.air_conditioning not in (
            "UNKNOWN",
            "",
        ):
            caps.append("CLIMATE")

        if state.auxiliary_heating not in (
            "UNKNOWN",
            "",
        ):
            caps.append("AUX_HEATING")

        if state.active_ventilation not in (
            "UNKNOWN",
            "",
        ):
    ...
    def _operations(state):
        ...
        return ",".join(
            str(x)
            for x in state.supported_operations
        )[:250]
    ...
    def update(
        ...
        state,
        api_status="OK",
        api_rate="Unavailable",
        api_key_expiry_days=None,
        vehicle_captured_elapsed=None,
        ...
        self.update_text(
            "parking_address",
            state.parking_address or "UNKNOWN",
        )

        if (
            state.parking_latitude is not None
            and state.parking_longitude is not None
        ):
            gps = "{}, {}".format(
                format_coordinate(
                    state.parking_latitude
                ),
                format_coordinate(
                    state.parking_longitude
                ),
            )
        else:
            gps = "UNKNOWN"
        ...
        self._update_custom_numeric(
            "vehicle_captured",
            vehicle_captured_elapsed,
            self.CUSTOM_SECONDS_OPTIONS,
        )

        self._update_custom_numeric(
            "api_key_expiry",
            api_key_expiry_days,
            self.CUSTOM_DAYS_OPTIONS,
            decimals=2,
        )
        ...
        self.update_text(
            "charging_captured",
            state.charging_captured_at or "UNKNOWN",
        )

        self.update_text(
            "fuel_captured",
            state.fuel_captured_at or "UNKNOWN",
        )

        self.update_text(
            "odometer_captured",
            state.odometer_captured_at or "UNKNOWN",
        )
        ...
        self.update_text(
            "api_errors",
            self._api_errors(state),
        )

        self.update_text(
            "supported_operations",
            self._operations(state),
        )
    ...
    def update_api_only(
        ...
        if cached_state is not None:
            self.update_text(
                "vehicle_security",
                self._smart_security(cached_state),
            )

            self.update_text(
                "climate_state",
                self._smart_climate(cached_state),
            )

            self.update_text(
                "vehicle_state",
                self._smart_vehicle_state(cached_state),
            )
